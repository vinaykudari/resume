...
\begin{tabular*}{\textwidth}{@{}l@{\extracolsep{\fill}}r@{}}
  ...
  \textit{Tech expertise in E-commerce, Healthcare, Gaming Industries \& Startups} & +1-469-943-6778 \\
\end{tabular*}
...
\section{Relevant skills}
	{\textbf{Python, SQL, JAVA}, \textit{FastAPI, LangGraph, PySpark, Kubernetes, Git, Redis, Kafka, Terraform}, \textbf{GCP, AWS}}
\vspace{-2pt}

\section{Experience}
  \resumeSubHeadingListStart
    \resumeSubheading{Apple}{Cupertino, CA}
    {Software Engineer  \(|\) \textbf{llm, generative-ai, agents, api-design}}{July 2024 - Present}
    \resumeItemListStart
        \resumeItemClean
         {Built a multi-agent system leveraging LangGraph and LLMs to convert natural language text into optimized SQL}
        \resumeItemClean
         {Developing a general-purpose agentic system to streamline analysis of documents and databases, significantly reducing manual review efforts}
        \resumeItemClean
          {Engineered a scalable full-stack financial analytics dashboard supporting real-time data streaming, enabling dynamic visualization and reporting}
       \resumeItemListEnd
  ...
  \resumeSubheading{Gap Inc}{San Francisco, CA}
    ...
    \resumeItemListStart
        ...
        \resumeItemClean
         {Facilitated migration of legacy cloud foundry systems to kubernetes leveraging github actions}
        \resumeItemClean
...
\section{Projects}
\resumeSubHeadingListStart
\resumeSubItem{Personal AI assistant (genai)}{Building a multi-agent AI companion with human-like memory simulation, capable of personalized conversations, context-aware recall, and tool use}
\vspace{2pt}

\resumeSubItem{Open-source contributions (nlp, open-source)}{Core contributor to HuggingFace Datasets official repository}
\vspace{2pt}

\resumeSubItem{Distributed key-value store (distributed systems)}{Developed redis-like distributed key-value store with a focus on achieving strong consistency and fault tolerance using the RAFT consensus algorithm}
\vspace{2pt}

\resumeSubItem{Full text search engine (search, nlp, information-retrieval)}{Developed a query parser to transform search query into boolean expression; Optimized the text retrieval by creating inverted index and performed batch processing.}
\vspace{2pt}
